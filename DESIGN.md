...
> 状态：已锁定（投影 / 范围 / 步进 / 主题 / 部署 / 数据 / 面积算法 / 图层 / 边缘策略均已用户确认）
...
| 主题 | **浅色默认** + 深色可切换 | 按钮文案「浅色 / 深色」 |
| 面板 | 侧栏可折叠；底栏预设可收，滑杆保留 | 状态存 localStorage |
| 地图边缘 | **经度无限环绕 + 纬度软钳制** | 不再露出硬黑块 |
| 叠加图层 | 国家界线、世界城市、中国行政区、大陆名 | 后三项可开关 |
...
| 上轮 | **产品名「海陆变迁」；浅色默认；侧栏/预设折叠；经度环绕+纬度软钳制；图层** |
| 本轮 | **中国地级行政区 333（无区县）；点名同生同灭；国家名；标签上限；实验档 ±8000；「关于与指南」弹层；底栏精简** |
| 最新 | **地级 5–10× 仅省会、≥10× 全量；冰盖取消回 0；滑杆滚轮 1 m；自然地名图层** |
